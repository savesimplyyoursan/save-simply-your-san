...
################################################################
	...
	shell_line =''
	    ...
	    response.append(shell_line) # join the output
...
def ParseUptime(switch, input):
	...
	#12:48pm   up for 473 days 2 mins
...
def ParseEnvironment(switch, input):
	...
	#switchStatusShow command looks like :
	#SwitchState:	HEALTHY
	#Duration:	23188:47
	...
	#Missing SFPs monitor  	HEALTHY
        temp_flag = False
	switchstate_line = [line for line in lines if line.startswith("SwitchState")]
	ps_line = [line for line in lines if line.startswith("Power supplies monitor")]
	temp_line = [line for line in lines if line.startswith("Temperatures monitor")]
	fan_line = [line for line in lines if line.startswith("Fans monitor")]
	flash_line = [line for line in lines if line.startswith("Flash monitor")]
	marginal_port_line = [line for line in lines if line.startswith("Marginal ports monitor")]
	faulty_port_line = [line for line in lines if line.startswith("Faulty ports monitor")]
	missing_SFP_line = [line for line in lines if line.startswith("Missing SFPs monitor")]
        if (not switchstate_line[0]) or (not ps_line[0]) or (not temp_line[0]) or (not fan_line[0]) or (not flash_line[0]) or (not marginal_port_line[0]) or (not faulty_port_line[0]) or (not missing_SFP_line[0]) :
            return False
        switch_state = ps_line[0].split()
	switch.switch_state = switch_state[-1]
	ps_state = ps_line[0].split()
	switch.ps_state = ps_state[-1]
	temp_state = temp_line[0].split()
	switch.temp_state = temp_state[-1]
	fan_state = fan_line[0].split()
	switch.fan_state = fan_state[-1]
	flash_state = flash_line[0].split()
	switch.flash_state = flash_state[-1]
	marginal_port_state = marginal_port_line[0].split()
	switch.marginal_port_state = marginal_port_state[-1]
	faulty_port_state = faulty_port_line[0].split()
	switch.faulty_port_state = faulty_port_state[-1]
	missing_SFP_state = missing_SFP_line[0].split()
	switch.missing_SFP_state = missing_SFP_state[-1]
	switch.fan_list = []
	switch.temp_list = []
	switch.ps_list = []
	
	#fanShow
	#Fan 1 is Ok, speed is 5400 RPM
	#Fan 2 is Ok, speed is 5578 RPM
	#Fan 3 is Ok, speed is 5532 RPM
        fans_line = [line for line in lines if ( line.startswith("Fan") and not line.startswith("Fans") )]
	if not fans_line:
	    return False
	for line in fans_line:
	    #print line
	    fan_list = line.split()
	    fan = number, status, rpm = fan_list[1], fan_list[3], fan_list[-2]
	    switch.fan_list.append(fan)
	#psShow
	#Power Supply #1 is OK
	#Power Supply #2 is OK
        ps_line = [line for line in lines if line.startswith("Power Supply #")]
	if not ps_line:
	    return False
	for line in ps_line:
	    #print line
	    ps_list = line.split()
	    ps = number, status = ps_list[2], ps_list[-1]
	    switch.ps_list.append(ps)
	#tempShow
	...
	#  2	Ok	    41		  105
	    ...
	    if line.startswith("Sensor"):
            ...
            if temp_flag:
	        if (not u"Sensor" in line) and not line.startswith("===") and not line.lstrip().startswith(u"ID") :
	             #print line
	             temp_list = line.split()
		     #print temp_list
	             if temp_list:
	                 temp = id, status, centigrade, fahrenheit = temp_list[0], temp_list[1], temp_list[2], temp_list[3]
		         switch.temp_list.append(temp)
...
def CheckEnv(switch, warning_threshold, critical_threshold):
	...
	env = switch.GetCommand(switch.environment_command)
        ...
        switch.fan_command = "fanShow"
        switch.ps_command = "psShow"
        switch.temp_command = "tempShow"
	env = switch.GetCommand(switch.environment_command)
	env += "\n"
	env += switch.GetCommand(switch.fan_command)
	env += "\n"
	env += switch.GetCommand(switch.ps_command)
	env += "\n"
	env += switch.GetCommand(switch.temp_command)
    elif switch.type == "mcdata":
	...
	if switch.type == "cisco":
	    ...
	    if ( nb_failed >= int(critical_threshold) ):
	        ...
	        output(STATE_WARNING, "WARNING: " + str(msg))
       	    ...
       	    nb_failed = 0
	    msg = []
	    if not switch.switch_state.startswith("HEALTHY"):
	        nb_failed += 1
		msg.append("Switch status is: "+ str(switch.switch_state) + " ! ")
	    else:
		msg.append("Switch status is: "+ str(switch.switch_state) + ". ")
	    if not switch.ps_state.startswith("HEALTHY"):
	        nb_failed += 1
		msg.append("Power Supplies status is: "+ str(switch.ps_state) + " ! ")
	    else:
		msg.append("Power Supplies is: "+ str(switch.ps_state) + ". ")
	    if not switch.temp_state.startswith("HEALTHY"):
	        nb_failed += 1
		msg.append("Temperatures status is: "+ str(switch.temp_state) + " ! ")
	    else:
		msg.append("Temperatures is: "+ str(switch.temp_state) + ". ")
	    if not switch.fan_state.startswith("HEALTHY"):
	        nb_failed += 1
		msg.append("Fans status is: "+ str(switch.fan_state) + " ! ")
	    else:
		msg.append("Fans is: "+ str(switch.fan_state) + ". ")
	    if not switch.flash_state.startswith("HEALTHY"):
	        nb_failed += 1
		msg.append("Flash monitor status is: "+ str(switch.flash_state) + " ! ")
	    else:
		msg.append("Flash monitor is: "+ str(switch.flash_state) + ". ")
	    if not switch.marginal_port_state.startswith("HEALTHY"):
	        nb_failed += 1
		msg.append("Marginal Ports status is: "+ str(switch.marginal_port_state) + " ! ")
	    else:
		msg.append("Marginal Ports is: "+ str(switch.marginal_port_state) + ". ")
	    if not switch.faulty_port_state.startswith("HEALTHY"):
	        nb_failed += 1
		msg.append("Faulty Ports status is: "+ str(switch.faulty_port_state) + " ! ")
	    else:
		msg.append("Faulty Ports is: "+ str(switch.faulty_port_state) + ". ")
	    if not switch.missing_SFP_state.startswith("HEALTHY"):
	        nb_failed += 1
		msg.append("Missing SFP status is: "+ str(switch.missing_SFP_state) + " ! ")
	    else:
		msg.append("Missing SFP is: "+ str(switch.missing_SFP_state) + ". ")

	    for (number, status, rpm) in switch.fan_list:
	        if not str(status).startswith("Ok") :
	            nb_failed += 1
		    msg.append("Fan "+str(number) + " is in status: " + str(status) + " (RPM: "+str(rpm)+ ") ! ")
	        else:
		    msg.append("Fan "+str(number) + " is: " + str(status) + " (RPM: "+str(rpm)+ "). ")
	    for (id, status, centigrade, fahrenheit) in switch.temp_list:
	        if not str(status).startswith("Ok") :
	            nb_failed += 1
		    msg.append("Temp "+str(id) + " is in status: " + str(status) + " ("+str(centigrade)+ " C) ! ")
	        else:
		    msg.append("Temp "+str(id) + " is: " + str(status) + " ("+str(centigrade)+ " C). ")
	    for (number, status) in switch.ps_list:
	        if not str(status).startswith("OK") :
	            nb_failed += 1
		    msg.append("Power Supply " + str(number) + " is in status: " + str(status) + " ! ")
	        else:
