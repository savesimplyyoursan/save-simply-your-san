...
"""
__author__ = "Anthony FOIGNANT <antispameu-san@yahoo.fr>" 
__version__ = "0.95" 
__version_info__ = (0, 95) 
__license__ = "GNU General Public License v2 (GPL2)"
...
class Switch():
	...
	@param zipped : output's files need to be zipped
	@type zipped: str
        """
	...
	if self.type == "brocade":
	    ...
	    raise self.InputError(self.type, "Not a good type of switch ! possibles values are : brocade or cisco or mcdata or others" )
...
#################################################
	...
	@type directory: str
        ...
        if not self.ChangeDir(directory):
            print "***SaveFileInDirectory*** Unable to go into the directory : "+str(directory)
            return False
	...
	"""
		...
		if self.zipped == 'yes':
		    ...
		    print "Successfully saved the file:"+str(name)
                ...
                print "***SaveFileInDirectory*** Unable to save the file: "+str(name)
		return False
	"""
...
################################################################
    def Connect(self):
	    ...
	    return self.TelnetConnect()
            ...
            raise self.InputError(self.connection_type, "Not a good connection type !")
...
#################################################################
	...
	if self.debug:
            print "SSHTestType output : " + output
            print "SSHTestType error : " + error
        response = []
        if error:
            if self.type == "brocade":
		...
		if not self.shell:
		    try:  
		        ...
		        print '***SSHTestType*** Unable to have a real SSH shell with this switch !'
		        self.client.close()
		        return False
		else:
		    shell = self.shell
...
#################################################################
	...
	if not self.shell:
		...
		self.shell = shell
                ...
                print '***SSHGetPromptAndType*** Unable to have a real SSH shell with this switch !'
                self.client.close()
                return False
	else:
	    shell = self.shell
	...
	time_start = time.time()
        while True:
			...
			try:
		            ...
		            print '*** SSHGetPromptAndType: Timeout when reading line: "%s" !' % str(shell_line)
			    self.client.close()
			    return False
		        if self.debug:
			    print "SSHGetPromptAndType shell line: '%s'" % str(shell_line)
 		        response.append(shell_line) #concatenate the output
			...
			if elapsed > self.prompt_timeout:
			    if not response:
	                        print "***SSHGetPromptAndType*** Prompt timeout ! No response for the prompt !"
	                        return False
			    else:
				response = ''.join(response)
                                if self.debug:
				    print 'SSHGetPromptAndType: Prompt timeout ! Got the response: "%s"' % str(response)
	                        if self.GetSwitchTypeFromPrompt(response):
                                    return True
			        else:
				    print '***SSHGetPromptAndType*** Impossible to guess the equipment type from the response: "%s"' % str(response)
			            return False
	"""
...
################################################################
	...
	"""
	    ...
	    if error:
	        ...
	        raise self.CommandError(error,"***SSHCommand*** Error with the command on the switch !")
	    ...
	    if not self.shell:   
		    ...
		    self.shell = shell
	            ...
	            self.client.close()
   	            raise self.ConnectionError("SSHCommand",'***SSHCommand*** Unable to have an SSH shell from the switch' )
            else:
                shell = self.shell
	    ...
	    ret = '\r'
		   ...
		   try:
		       ...
		       self.client.close()
		       raise self.ConnectionError("SSHCommand",'SSHCommand: Timeout when reading line: "%s" !' % str(shell_line))
	           if self.debug:
		       print "SSHCommand shell line: '%s' " % str(shell_line)
		   output.append(shell_line) #concatenate the output
	           if ("more" in shell_line.lower()) or ("<cr>" in shell_line.lower()):
...
################################################################
    def SSHSave(self, server_queue, server_thread):
        """
        SSHSave(self,server_queue, server_thread) -> None
	...
	shellfile = shell.makefile('rw')
	ret = '\r' # defining the return caracter
	server_thread.start() # starting the Server
        # sending the commandtest to the switch
	#time.sleep(2)
	...
	while True:
		...
		if self.prompt in shell_line:
		    ...
		    if self.debug:
		        print "SSHSave commandseen=True"
                    commandseen = True
		    pass
                if commandseen:
                    if 'authenticity' in shell_line.lower():
			...
			shellfile.write('yes' + ret)
                    ...
                    elif 'assword' in shell_line.lower():
                        print "sending transfert's password"
			shellfile.write(self.transfert_password  + ret)
                        shellfile.flush()
                        break
                    elif ('refused' in shell_line.lower()) or ('failed'in shell_line.lower()) :
			print '***SSHSave*** Connection refused by the SSH or FTP Server : ' + shell_line
			server_thread._Thread__stop()
                        self.client.close()
			raise self.ConnectionError('***SSHSave***','Connection refused by the SSH or FTP Server !')
	...
	try:
        ...
        if result == 'backup OK':
            print 'Transfert and Backup successfully done !'
	    server_thread._Thread__stop()
	    self.client.close()
            return True
        else:
...
def GenerateServerKey(type='RSA', keylength=1024):
	...
	raise self.InputError("*** GenerateServerKey ***", "Not a good type of key. Possible values are RSA or DSS !")
...
def SSHserver_launch(switch, server_queue, timeout):
	...
	if switch.server_key_type == 'RSA':
	    ...
	    raise self.SCPError("SSHServer","Wrong type of SSH key. Possible values are RSA or DSS !")
	...
	try:
		...
		print '***SSHServer*** Could not bind on port 22' + str(e)
    		traceback.print_exc()
    		raise self.SCPError("SSHServer","Could not bind on port 22 !")
	...
	try:
		...
		print 'SSHServer : Listening for connection ...'
    		...
    		traceback.print_exc()
    		raise self.SCPError("SSHServer", "Listen/accept failed !")
	try:
...
class FTPServer(threading.Thread):
	...
	def __init__(self, switch, server_queue, timeout):
		...
		# Changing the working directory
		    ...
		    self.server_queue.put('backup FAIL')
                    raise self.FTPError("FTPServer", "Unable to go the directory with this switch's name")
		...
		if (os.path.isfile(self.file)):
			...
			raise self.FTPError("FTPServer", "Unable to rename the file")
	...
	def FTPquit(self, ftpd):
		if os.path.isfile(self.file):
		    print 'FTPServer : Configuration backup done'
		    if self.zipped == 'yes':
		        zname = str(self.file) + '.zip'
		        zbackup = zipfile.ZipFile(zname, "w")
		        zbackup.write(name, name, ZIP_DEFLATED)
		        zbackup.close()
		        os.remove(self.file)
