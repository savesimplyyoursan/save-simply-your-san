...
def SaveSimplyyourSAN_CLI():
	...
	parser.add_option("-e", "--execute", help="Execute the given command on the equipment. ", type="string", action="store", dest="command")
	...
	parser.add_option("--known_hosts", help="The known_hosts file.", type="string", action="store", dest="known_hosts", default="known_hosts")
	...
	if options.debug == 'yes':
	    ...
	    options.debug = False
	switch = SaveSimplyyourSAN.Switch(options.ip, options.user, options.password, options.switch, options.client, options.transfert, float(options.timeout), options.interface, options.nat, options.dir, debug=options.debug, known_hosts=options.known_hosts, server_key=options.server_key, server_key_type=options.server_key_type)
	#if a command is provided we connect to the switch and grab the output of the command
	if options.command:
	    if switch.Connect():
		...
		output = switch.GetCommand(options.command)
	        ...
	        switch.client.close()
	        if switch.SaveFileInDirectory(output, filename, switch.name):
	            sys.exit(0)
            else:
	...
	if switch.type == "mcdata":
		...
		if switch.Connect():
			...
			print "*** Error with the type of switch !"
			sys.exit(2)
		    if (switch.type != "brocade") and (switch.type != "cisco"):
			print "*** Error impossible to launch a server with a non brocade or cisco equipment !"
			sys.exit(2)
